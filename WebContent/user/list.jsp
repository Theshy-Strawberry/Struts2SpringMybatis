    	...
    	<fieldset>
            ...
            <s:if test="%{#request.pager.total > 0}">
					...
					<div id="pagination" class="pagination" >
						<ul>
							<pg:pager url="${contextPath}/front/user_index.action" items="${pager.total}"	
...
$(function(){
	$("#add-btn").click(function(){
		window.self.location = "${contextPath}/user/add.jsp";
	});
	$(".button-flat-highlight").click(function(){
		...
		$.dialog({
		    ...
		    ok: function () {
		    	/**确认信息*/
		    	$.post("${contextPath}/front/user_delete.action",{
					"user.tbid":tbid
				},function(msg){
					if(msg == 1){
						refresh();
	...
	$(".button-flat-primary").click(function(){
		var tbid = $(this).attr("tbid");
		window.self.location = "${contextPath}/front/user_intoUpdate.action?user.tbid="+tbid;
	});
});
//刷新当前页面
function refresh(){
	window.self.location = "${contextPath}/front/user_index.action?pager.offset=${pager.offset}";
}
</script>
</body>
</html>